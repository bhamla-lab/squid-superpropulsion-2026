% C02: Side view - Area and Intensity vs Normalized Time
...
if isempty(allMetrics)
    error('No SideView metrics files found.');
end

fprintf('Found %d SideView metrics files\n\n', length(allMetrics));
...
title('Green Dye Area vs Normalized Time (Side View)', 'FontSize', 14);
...
title('Green Dye Intensity vs Normalized Time (Side View)', 'FontSize', 14);
legend('Location', 'best');
grid on;

sgtitle('Side View: Mixing Metrics by Nozzle Type', 'FontSize', 16, 'FontWeight', 'bold');

fprintf('\nVisualization complete (Side View)!\n');
