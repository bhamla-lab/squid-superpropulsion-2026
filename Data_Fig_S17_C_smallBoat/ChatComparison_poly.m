...
for c_idx = 1:num_conditions
    ...
    fit_params = process_condition(cond, files, trials, ...
        ...
        poly_order, colors_map(c_idx,:), colors, fit_params);
    plot_idx = plot_idx + 1;
end
...
%% Figure 2: Power Traces - SKIPPED (No power data available)
% Commenting out entire Figure 2 section - no power data in current dataset
...
for c_idx = 1:num_conditions
    ...
    if isfield(fit_params, field) && isfield(fit_params.(field), 'r_squared')
        ...
        % Print individual trial R² values
        if isfield(fit_params.(field), 'trial_r_squared')
            trial_r2 = fit_params.(field).trial_r_squared;
            for i = 1:length(trial_r2)
                fprintf('  Trial %d: R² = %.5f', trials(i), trial_r2(i));
                if trial_r2(i) < 0.99
                    fprintf(' ⚠');
                end
                fprintf('\n');
...
fprintf('Note: R² > 0.99 indicates excellent fit\n');
...
function fit_params = process_condition(condition, files, trials, ...
    traj_folder, video_fps, pixels_per_mm, bl_mm, target_time, poly_order, ...
    color, colors, fit_params)
    ...
    trial_r_squared = [];   % Store R² for each trial
        ...
        if ~isempty(file_idx)
            ...
            if ~isempty(X)
                ...
                if max(time) >= target_time
                    ...
                    % Calculate R² for this trial
                    x_pred_trial = polyval(p_trial_norm, t_trial_norm);
                    ss_res_trial = sum((x_trial_norm - x_pred_trial).^2);
                    ss_tot_trial = sum((x_trial_norm - mean(x_trial_norm)).^2);
                    r2_trial = 1 - ss_res_trial/ss_tot_trial;
                    trial_r_squared = [trial_r_squared; r2_trial];
    ...
    if ~isempty(all_time)
        ...
        fit_params.(field_name).trial_r_squared = trial_r_squared;  % Store individual trial R²
        ...
        if target_time <= max(all_time) && length(trial_velocities) >= 1
            ...
            % Add annotation with individual trial R² values
            r2_str = '';
            for i = 1:length(trial_r_squared)
                r2_str = [r2_str sprintf('R²(T%d): %.4f\n', trials(i), trial_r_squared(i))];
            end
            r2_str = [r2_str sprintf('R²(avg): %.4f', r_squared)];

            if length(trial_velocities) >= 2
                text(0.98, 0.98, sprintf('v: %.2f±%.2f BL/s\na: %.3f±%.3f BL/s²\n%s', ...
                    v_at_target_bl, fit_params.(field_name).v_at_target_std_bl, ...
                    accel_bl, fit_params.(field_name).accel_std_bl, r2_str), ...
                    'Units', 'normalized', 'HorizontalAlignment', 'right', ...
                    'VerticalAlignment', 'top', 'BackgroundColor', 'w', ...
                    'EdgeColor', 'k', 'FontSize', 9, 'Margin', 3);
            else
                text(0.98, 0.98, sprintf('v: %.2f BL/s\na: %.3f BL/s²\n%s', ...
                    v_at_target_bl, accel_bl, r2_str), ...
                    'Units', 'normalized', 'HorizontalAlignment', 'right', ...
                    'VerticalAlignment', 'top', 'BackgroundColor', 'w', ...
                    'EdgeColor', 'k', 'FontSize', 9, 'Margin', 3);
            end
